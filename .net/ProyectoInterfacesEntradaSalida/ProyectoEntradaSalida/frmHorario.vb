Public Class frmHorario
    ...
    Private Sub frmHorario_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim lunes As List(Of String) = rellenarDiasSemana(fichas)
        fichas = restApi.fichaHorario(DateAdd("d", 1, fecha))
        Dim martes As List(Of String) = rellenarDiasSemana(fichas)
        fichas = restApi.fichaHorario(DateAdd("d", 1, fecha))
        Dim miercoles As List(Of String) = rellenarDiasSemana(fichas)
        fichas = restApi.fichaHorario(DateAdd("d", 1, fecha))
        Dim jueves As List(Of String) = rellenarDiasSemana(fichas)
        fichas = restApi.fichaHorario(DateAdd("d", 1, fecha))
        Dim viernes As List(Of String) = rellenarDiasSemana(fichas)

        rellenarTabla(lunes, martes, miercoles, jueves, viernes)
    End Sub

    Function rellenarDiasSemana(fichas As List(Of Ficha)) As List(Of String)
        Dim list As List(Of String) = New List(Of String)
        For i As Integer = 0 To 6
            list.Add("-----")
        Next
        For Each ficha As Ficha In fichas
            If ficha.horario.horaInicio.Equals("08:00 h") Then
                list(0) = ficha.horario.aula.nombre
            ElseIf ficha.horario.horaInicio.Equals("09:00 h") Then
                list(1) = ficha.horario.aula.nombre
            ElseIf ficha.horario.horaInicio.Equals("10:00 h") Then
                list(2) = ficha.horario.aula.nombre
            ElseIf ficha.horario.horaInicio.Equals("11:00 h") Then
                list(3) = ficha.horario.aula.nombre
            ElseIf ficha.horario.horaInicio.Equals("12:00 h") Then
                list(4) = ficha.horario.aula.nombre
            ElseIf ficha.horario.horaInicio.Equals("13:00 h") Then
